...
const k=new THREE.Group();k.userData.baseY=mini?0:-.34;k.scale.setScalar(mini?.64:.86);k.position.y=k.userData.baseY;root.add(k);
...
if(!mini){canvas.addEventListener('pointerdown',e=>{drag=true;lastX=e.clientX;canvas.setPointerCapture(e.pointerId)});canvas.addEventListener('pointermove',e=>{if(!drag)return;rot+=(e.clientX-lastX)*.008;lastX=e.clientX});canvas.addEventListener('pointerup',()=>drag=false);canvas.addEventListener('pointercancel',()=>drag=false)}
